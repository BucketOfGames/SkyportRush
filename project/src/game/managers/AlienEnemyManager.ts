...
export class AlienEnemyManager {
  ...
  private getEnemyHeightOffset(type: 'scout' | 'warrior' | 'heavy' | 'flyer'): number {
    switch (type) {
      case 'scout': return 0.8; // Sphere radius
      case 'warrior': return 1.25; // Half box height
      case 'heavy': return 1.5; // Half box height
      case 'flyer': return 1; // Cone base radius
      default: return 1;
    }
  }
  ...
  private updateEnemyTerrainCollision(enemy: AlienEnemy): void {
    if (enemy.type === 'flyer') {
      // Flyers hover above terrain
      const terrainHeight = this.getTerrainHeightAtPosition(
        enemy.mesh.position.x, 
        enemy.mesh.position.z
      );
      const targetHeight = terrainHeight + 8 + Math.sin(Date.now() * 0.003) * 2;
      enemy.mesh.position.y = THREE.MathUtils.lerp(enemy.mesh.position.y, targetHeight, 0.1);
    } else {
